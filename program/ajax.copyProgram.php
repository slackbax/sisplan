...
if (extract($_POST)):
    ...
    try {
        ...
        foreach ($dist as $i => $d):
            ...
            if ($chk == 0):
                $ins = $di->set($d->disp_pesid, $d->disp_descripcion, $date, $date_t, $d->disp_jusid, $d->disp_serid, $d->disp_espid, $d->disp_vacaciones, $d->disp_permisos, $d->disp_congreso, $d->disp_descanso, $d->disp_med_general, $_SESSION['prm_userid'], $db);
                ...
                foreach ($horas as $v => $h):
                    $ins_h = $dh->set($ins['msg'], $h->dhp_acpid, $h->dhp_cantidad, $h->dhp_rendimiento, $db);
