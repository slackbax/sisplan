$(document).ready(function () {
	...
	function calTHoras(id) {
		...
		if (tHoras > 0) {
			...
			if (tHoras > hDisp) {
				swal({
					title: "Error de Programación",
					text: "La suma de horas programadas (" + tHoras + ") es mayor que las horas disponibles ingresadas (" + hDisp + ").",
					type: "error",
					showCancelButton: false,
					confirmButtonText: "Aceptar"
				});
	...
	$('#iNesp').change(function () {
		...
		$('.proces .glyphicon').each(function () {
			$(this).removeClass('fa-check');
		});
		...
		if ($(this).val() !== '') {
			$.ajax({
				...
				dataType: 'json',
				data: {d: $('#iNdate').val(), esp: $(this).val(), serv: $('#iNserv').val(), per: $('#iid').val()}
			}).done(function (data) {
				if (data.dia_id !== null) {
