...
if (extract($_POST)):
	try {
		...
		if ($iplanta == 0):
			$obj->setCellValue('E1', 'ID_Especialidad');
			$obj->setCellValue('F1', 'Especialidad');
		else:
			$obj->setCellValue('E1', 'ID_Profesion');
			$obj->setCellValue('F1', 'Profesion');
		endif;
		$obj->getColumnDimension('F')->setWidth(52);
		$obj->setCellValue('G1', 'Id_Actividad');
		$obj->setCellValue('H1', 'Actividad');
		$obj->getColumnDimension('H')->setWidth(60);
		$obj->setCellValue('I1', 'Horas Asignadas');
		$obj->setCellValue('J1', 'Rendimiento por Hora');
		$obj->setCellValue('K1', 'Total Horas semanales del contrato');
		$obj->setCellValue('L1', '% asignado del contrato');
		$obj->setCellValue('M1', 'Total horas semanales contratadas RUN');
		$obj->setCellValue('N1', 'VC');
		$obj->setCellValue('O1', 'V Horas legales descuento (Congreso o capacitación + Descanso complementario + F. Legales + lactancia + Colación + Permisos administrativos) (Proporcional)');
		$obj->getColumnDimension('O')->setWidth(32);
		$obj->setCellValue('P1', 'Producción Estimada Anual');
		$obj->setCellValue('Q1', 'M. General');
		$obj->setCellValue('R1', 'Requiere rendimiento');
		...
		foreach ($ds as $in => $dat):
			...
			if ($iplanta == 0):
				$obj->setCellValue('E' . $i, $dat->esp_codigo);
				$obj->setCellValue('F' . $i, $dat->esp_descripcion);
			else:
				$obj->setCellValue('E' . $i, $dat->prof_id);
				$obj->setCellValue('F' . $i, $dat->prof_descripcion);
			endif;
			$obj->setCellValue('G' . $i, $dat->act_id);
			$obj->setCellValue('H' . $i, $dat->act_desc_reporte);
			$obj->setCellValue('I' . $i, $dat->act_horas);
			$obj->setCellValue('J' . $i, $dat->act_rend);
			$obj->setCellValue('K' . $i, $cont->pes_horas);

			$perc = $dat->act_horas * 100 / $cont->pes_horas;
			$obj->setCellValue('L' . $i, number_format($perc, 0, '.', ''));

			$total = $pe->getTotalContratos($dat->per_id, $dat->est_id);
			$obj->setCellValue('M' . $i, $total);

			$medg = ($dat->per_medgeneral == 0) ? 'No' : 'Sí';
			$obj->setCellValue('Q' . $i, $medg);

			$rend = ($dat->act_rend == 0) ? 'NR' : 'R';
			$obj->setCellValue('R' . $i, $rend);
		...
		$obj->getStyle('A2:R' . $i)->applyFromArray($styleArray);
