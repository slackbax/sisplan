...
class SubEspecialidadSin {
	...
	public function getAll($db = null) {
		...
		$stmt = $db->Prepare("SELECT ssub_id FROM prm_sin_subespecialidad ORDER BY ssub_nombre ASC");
	 ...
	 * @return array
	 */
	public function getByEspecialidad($id, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;

		$stmt = $db->Prepare("SELECT ssub_id FROM prm_sin_subespecialidad WHERE sesp_id = ? ORDER BY ssub_nombre ASC");
