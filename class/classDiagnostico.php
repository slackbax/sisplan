...
class Diagnostico {
	 ...
	 * @return stdClass
	 */
	public function getByEspDate($est, $esp, $serv, $date, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	 ...
	 * @return array
	 */
	public function set($est, $esp, $ser, $us, $fecha, $total, $lista, $total_iq, $lista_iq, $disp_ata, $disp_atc, $disp_pro, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
		...
		try {
			...
			$bind = $stmt->bind_param("iiiisiiiiiii", $db->clearText($est), $db->clearText($esp), $db->clearText($ser), $db->clearText($us), $db->clearText($fecha), $db->clearText($total), $db->clearText($lista), $db->clearText($total_iq),
				$db->clearText($lista_iq), $db->clearText($disp_ata), $db->clearText($disp_atc), $db->clearText($disp_pro));
			if (!$bind):
