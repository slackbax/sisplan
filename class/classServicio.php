...
class Servicio {
	...
	public function getAll($db = null) {
		...
		$stmt = $db->Prepare("SELECT ser_id FROM prm_servicio ORDER BY ser_nombre ASC");
		$stmt->execute();
		$result = $stmt->get_result();
		$lista = [];
	 ...
	 * @return array
	 */
	public function getByCR($id, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;

		$stmt = $db->Prepare("SELECT ser_id FROM prm_servicio WHERE cr_id = ? ORDER BY ser_nombre ASC");
		$stmt->bind_param("i", $id);
		$stmt->execute();
		$result = $stmt->get_result();
		$lista = [];
