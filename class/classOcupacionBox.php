...
class OcupacionBox {
	...
	public function getAll($db = null) {
		...
		$stmt = $db->Prepare("SELECT b.bh_id FROM prm_bloque_hora b ORDER BY b.bh_hora_ini ASC");
	...
	public function getOccupation($box, $fecha, $db = null) {
		...
		$stmt = $db->Prepare("SELECT bh_id
									FROM prm_bloque_hora bh
									WHERE bh.box_id = ? and bh.bh_fecha = ? AND bh.bles_id = 1 
									GROUP BY bh.bh_id
									ORDER BY bh.bh_fecha, bh.bh_hora_ini ASC");
	 ...
	 * @return array
	 */
	public function getOccupationByBoxFechaHora($box, $fechai, $hora, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
		...
		$stmt = $db->Prepare("SELECT bh.bh_id 
									FROM prm_bloque_hora bh
									WHERE bh.box_id = ? and bh.bh_fecha = ? AND bh.bh_hora_ini = ? AND bh.bles_id = 1
									GROUP BY bh.bh_id
									ORDER BY bh.bh_fecha, bh.bh_hora_ini ASC");

		$stmt->bind_param("iss", $box, $fechai, $hora);
		$stmt->execute();
		$result = $stmt->get_result();
		$lista = [];
	 ...
	 * @return array
	 */
	public function getOccupationFloorByFecha($floor, $fechai, $fechat, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
		...
		$stmt = $db->Prepare("SELECT bh.bh_id
									FROM prm_bloque_hora bh
									JOIN prm_box bx ON bh.box_id = bx.box_id
									WHERE bx.lug_id = ? and bh.bh_fecha BETWEEN ? AND ? AND bh.bles_id = 1
									GROUP BY bh.bh_id
									ORDER BY bh.bh_fecha, bh.bh_hora_ini ASC");

		$stmt->bind_param("iss", $floor, $fechai, $fechat);
		$stmt->execute();
		$result = $stmt->get_result();
		$lista = [];
