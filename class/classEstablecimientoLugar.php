...
class EstablecimientoLugar {
	...
	public function getAll($db = null) {
		...
		$stmt = $db->Prepare("SELECT lug_id FROM prm_estab_lugar ORDER BY lug_nombre ASC");
	 ...
	 * @return array
	 */
	public function getByEstab($est, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;

		$stmt = $db->Prepare("SELECT lug_id FROM prm_estab_lugar WHERE est_id = ? ORDER BY lug_nombre ASC");
