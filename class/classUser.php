...
class User {
	...
	public function getAll($db = null) {
		...
		$stmt = $db->Prepare("SELECT us_id FROM prm_usuario WHERE us_existe = TRUE ORDER BY us_ap ASC, us_am ASC, us_nombres ASC");
	 ...
	 * @return array
	 */
	public function getGroups($id, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	...
	public function mod($id, $name, $ap, $am, $email, $pass, $active, $db = null) {
		...
		try {
			...
			$bind = $stmt->bind_param("sssssii", utf8_decode($db->clearText($name)), utf8_decode($db->clearText($ap)), utf8_decode($db->clearText($am)), $db->clearText($email), $txt_p, $active, $id);
			if (!$bind):
