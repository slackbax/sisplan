...
class Persona {
	 ...
	 * @return array
	 */
	public function getByPlanta($est, $planta, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	 ...
	 * @return stdClass
	 */
	public function getByRutEstLey($rut, $est, $ley, $corr, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
		...
		$stmt = $db->Prepare("SELECT * FROM prm_persona p
                                    ...
                                    WHERE p.per_rut = ? AND pe.con_id = ? AND pe.est_id = ? AND pe.pes_correlativo = ?");
	 ...
	 * @return stdClass
	 */
	public function getByRec($rec, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	 ...
	 * @return mixed
	 */
	public function getNPByDate($date, $planta, $est, $npr, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
		...
		$str_npr = ($npr == 'on') ? "AND pes_id NOT IN (SELECT pes_id FROM prm_distribucion WHERE dist_fecha = ?)" : "";
	 ...
	 * @return mixed
	 */
	public function getNPrByDate($date, $date_t, $planta, $est, $npr, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	 ...
	 * @return array
	 */
	public function getNPMedics($est, $date, $planta, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	 ...
	 * @return array
	 */
	public function getJustifyMeds($est, $date_i, $date_t, $planta, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	...
	public function getByString($string) {
		...
		$bind = $stmt->bind_param("ssss", $str, $str, $str, $str);
		$stmt->execute();
		if (!$stmt):
	 ...
	 * @return array
	 */
	public function mod($id, $nombre, $prof, $espec, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
