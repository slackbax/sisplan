...
class Group {
	...
	public function getAll($db = null) {
		...
		$stmt = $db->Prepare("SELECT gr_id FROM prm_grupo WHERE gr_existe = TRUE ORDER BY gr_id ASC");
	 ...
	 * @return array
	 */
	public function getIsEmpty($id, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	 ...
	 * @return array
	 */
	public function existsGroup($group, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
