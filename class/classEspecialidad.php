...
class Especialidad {
	...
	public function getAll($db = null) {
		...
		$stmt = $db->Prepare("SELECT esp_id FROM prm_especialidad ORDER BY esp_nombre ASC");
	 ...
	 * @return array
	 */
	public function getByServicio($serv, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
	 ...
	 * @return array
	 */
	public function getNoDiagnose($date, $est, $serv, $db = null) {
		if (is_null($db)):
			$db = new myDBC();
		endif;
