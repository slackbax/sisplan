...
$fileName = '../../upload/data_multi.xlsx';
...
foreach ($reader->getSheetIterator() as $sheet):
    foreach ($sheet->getRowIterator() as $row):
        ...
        if(!$head):
            ...
            if (isset($row[62]) and !empty(trim($row[62])) and substr(trim($row[62]), -3) == 'A07'):
                $count_meds++;
                $rut = number_format( intval(trim($row[45])), 0, '', '.' ) . '-' . trim($row[46]);
				$prof = trim(substr(trim($row[62]), 0, -4));
                $act = trim($row[10]);
                $tipoat = trim($row[58]);
                ...
                try {
                    ...
                    if (is_null($per->per_id)):
                        ...
                        $ins_p = $p->set($rut, $profe->prof_id, trim($row[47]), trim($row[48]), trim($row[49]), '', $db);
                        
                        $pers[] = array( 'rut' => $rut, 'profesion' => $profe->prof_id, 'nombre' => trim($row[47]), 'ap' => trim($row[48]), 'am' => trim($row[49]), 'rec' => '' );
