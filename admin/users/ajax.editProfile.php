...
if (extract($_POST)):
    ...
    try {
        ...
        if (!empty($_FILES)):
            $targetFolder = '/sisplan/image/users/';
            $targetPath = $_SERVER['DOCUMENT_ROOT'] . $targetFolder;

            $u = $user->get($id);
            
            if ($u->us_pic == 'image/users/no-photo.png'):
                $_default = true;
            endif;
            
            $img_old = $_SERVER['DOCUMENT_ROOT'] . '/sisplan/' . $u->us_pic;
            ...
            $pic_route = 'image/users/' . $id . '_' . $file['name'][0];
            ...
            $_SESSION['prm_userpic'] = 'image/users/' . $id . '_' . $file['name'][0];
        endif;
