 ...
 * @return string
 */
function getDateOnlyMonthToForm($d) {
	$aux = explode('-', $d);
	return $aux[1];
}
 ...
 * @return string
 */
function getDateOnlyHourToForm($d) {
	$aux = explode(':', $d);
	$num = (float) $aux[0];
...
function getArrayDate($d) {
	...
	$year = date("Y", $date);
	$a_date = array('day_w' => $day_w, 'day' => $day, 'month_w' => $month_w, 'month' => $month, 'year' => $year);
	return $a_date;
}
 ...
 * @throws Exception
 */
function daysBetweenDates($start, $end, $format = 'Y-m-d') {
 ...
 * @throws Exception
 */
function workingDaysBetweenDates($start, $end, $format = 'Y-m-d') {
	$start = new DateTime($start);
	$end = new DateTime($end);
	$invert = $start > $end;
 ...
 * @return stdClass
 */
function firstLastWeekDay($format = 'Y-m-d') {
	$date = new stdClass();
