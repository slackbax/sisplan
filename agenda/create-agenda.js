$(document).ready(function () {
	...
	$('#hora-fin').timepicker('setTime', '08:30 AM');

	$('#hora-inicio').timepicker().on('hide.timepicker', function (e) {
		var meridian = '', hour = '', minute = '';
		if (e.time.hours < 8) {
			swal({
				title: "Error!",
		...
		if (e.time.hours === 17) {
			if (e.time.minutes === 30) {
				...
				$('#hora-inicio').timepicker('setTime', '17:00 PM');
			}
		}
		if (e.time.hours > 17) {
			swal({
		...
		if ($('#hora-fin').data('timepicker').hour === e.time.hours) {
			if ($('#hora-fin').data('timepicker').minute === e.time.minutes) {
				hour = $('#hora-fin').data('timepicker').hour;
				minute = $('#hora-inicio').data('timepicker').minute + 30;
				if (minute === 60) {
					minute = 0;
					hour++;
				}
				...
				$('#hora-fin').timepicker('setTime', hour + ':' + minute + ' ' + meridian);
			} else if ($('#hora-fin').data('timepicker').minute < e.time.minutes) {
				hour = $('#hora-inicio').data('timepicker').hour + 1;
				minute = $('#hora-inicio').data('timepicker').minute - 30;
				if (minute === 60) {
					minute = 0;
					hour++;
		...
		} else if ($('#hora-fin').data('timepicker').hour < e.time.hours) {
			hour = $('#hora-inicio').data('timepicker').hour;
			minute = $('#hora-inicio').data('timepicker').minute + 30;
			if (minute === 60) {
				minute = 0;
				hour++;
	...
	$('#hora-fin').timepicker().on('hide.timepicker', function (e) {
		console.log(e);
		...
		if (e.time.hours === 18) {
			if (e.time.minutes === 30) {
				...
				$('#hora-fin').timepicker('setTime', '18:00 PM');
			}
		}
		if (e.time.hours > 18) {
			swal({
		...
		if ($('#hora-inicio').data('timepicker').hour === e.time.hours) {
			if ($('#hora-inicio').data('timepicker').minute === e.time.minutes) {
				hour = $('#hora-fin').data('timepicker').hour - 1;
				minute = $('#hora-fin').data('timepicker').minute + 30;
				...
				hour = (hour < 10) ? '0' + hour : hour;
				meridian = (hour > 11) ? 'PM' : 'AM';
				$('#hora-inicio').timepicker('setTime', hour + ':' + minute + ' ' + meridian);
			} else if ($('#hora-inicio').data('timepicker').minute > e.time.minutes) {
				hour = e.time.hours + 1;
				minute = 0;
		...
		} else if ($('#hora-inicio').data('timepicker').hour > e.time.hours) {
			hour = $('#hora-fin').data('timepicker').hour - 1;
			minute = $('#hora-fin').data('timepicker').minute + 30;
	...
	$('#calendar').fullCalendar({
		...
		eventRender: function (event, element) {
			...
			if (event.eventSource !== 'ajax') {
				element.append("<div style='position:absolute;bottom:0px;right:0px;z-index:10' class='closon'><button type='button' id='btnDeleteEvent' class='btn btn-xs btn-default' style='font-size:8px;border-radius:50%;'>X</button></div>");
			}
	...
	$('#cupos-cont').on('change', '.numcupos', function () {
		...
		if (!t_c) $('#add-new-event').prop('disabled', true);
	});
		...
		if ($(this).val() !== '') {
			...
			$.ajax({
				...
				$.each(data, function (k, v) {
					$('#boxat-id').append(
						$('<option></option>').val(v.box_id).html(v.box_numero)
					);
				});
			});
		}
	...
	$('#btnnext').click(function () {
		if (numActividades > 0 && actPendientes === 0) {
			...
			var msg = 'Por favor, resuelva los siguientes problemas:<br><br>';
